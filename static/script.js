...
function showRecommendation(event) {
  ...
  // UI glow, ripple, background animation
  ...
  // Show loading text
  output.innerHTML = `<p class='text-sm text-gray-400 animate-fade-in'>Fetching recommendations...</p>`;

  // 🔌 Send real POST to Flask
  fetch("/recommend", {
      method: "POST",
  ...
  .then(data => {
      if (data.error) {
          output.innerHTML = `<p class="text-red-500 text-sm">Error: ${data.error}</p>`;
      } else {
          output.innerHTML = `
              <div class="animate-fade-in">
                  <h3 class="text-lg font-medium mb-2">🎵 Top Recommendations</h3>
                  <ul class="text-sm text-left list-disc ml-6">
                      ${data.map(song => `<li>${song.track_name} — ${song.artists}</li>`).join('')}
                  </ul>
  ...
  .catch(err => {
      ...
      output.innerHTML = `<p class="text-red-500 text-sm">Failed to connect to server.</p>`;
  });
}
